...
const T = {
  ro: {
    subject: 'Bun venit la MoldMasters! 🎉',
    html: (ctx: TemplateContext) => `
    <p>Bună ziua${ctx.userName ? `, ${ctx.userName}` : ''}!</p>
    <p>Suntem bucuroși că v-ați alăturat MoldMasters — platforma pentru găsirea celor mai buni meșteri din Moldova.</p>
  ...
  en: {
    subject: 'Welcome to MoldMasters! 🎉',
    html: (ctx: TemplateContext) => `
    <p>Hello${ctx.userName ? `, ${ctx.userName}` : ''}!</p>
    <p>We're glad you joined MoldMasters — the platform for finding the best masters in Moldova.</p>
