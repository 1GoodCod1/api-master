 ...
 *
 * That upgrade only requires:
 * 1. npm install @nestjs/bull bull ioredis
 * 2. Replace Map<> with this.queue.add(...)
 * 3. Add @Process decorator to the handler methods
 */
@Injectable()
export class ExportQueueService {
    ...
    enqueue(
        ...
        this.logger.log(`Export job enqueued: ${job.id} (${type}, master=${masterId})`);
        return job;
    }
    ...
    private async generate(
        ...
        if (job.type === 'csv') {
            const data = await this.exportService.exportLeadsToBuffer(job.masterId, user, 'csv');
            return {
                data,
        ...
        if (job.type === 'excel') {
            const data = await this.exportService.exportLeadsToBuffer(job.masterId, user, 'excel');
            return {
                data,
        ...
        if (job.type === 'pdf') {
            const data = await this.exportService.exportAnalyticsToBuffer(job.masterId, user);
            return {
                data,
